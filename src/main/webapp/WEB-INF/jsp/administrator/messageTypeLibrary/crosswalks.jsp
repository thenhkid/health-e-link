        ...
        <c:choose>
            <c:when test="${availableCrosswalks.size() > 0}">
                <c:forEach items="${availableCrosswalks}" var="crosswalk" varStatus="pStatus">
                    <tr>
                        <td scope="row">
                            ${availableCrosswalks[pStatus.index].id} - ${availableCrosswalks[pStatus.index].name} <c:choose><c:when test="${availableCrosswalks[pStatus.index].orgId == 0}"> (generic)</c:when><c:otherwise> (Org Specific)</c:otherwise></c:choose>
                        </td>
